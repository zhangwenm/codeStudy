...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	protected void publishEvent(Object event, @Nullable ResolvableType eventType) {
		...
		// Multicast right now if possible - or lazily once the mul		if (this.earlyApplicationEvents != null) {
		//			this.earlyApplicationEvents.add(applicationEvent);
		//		}
		//		else {
		//			getApplicationEventMulticaster().multicastEvent(applicationEvent, eventType);
		//		}ticaster is initialized
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			//3:对bean工厂进行填充属性，注册解析解析接口方式的监听器
			...
			try {
				...
				// 首先实例化内置的创世纪处理器解析我们的配置类. 1. 会在此将class扫描成beanDefinition  2.bean工厂的后置处理器调用
	...
	protected void registerListeners() {
		...
		//获取bean定义中的监听器对象（接口方式的监听器），
		// 防止懒加载（懒加载不会再容器初始化的时候创建，即不会走后置处理器的解析）导致的监听器漏掉
