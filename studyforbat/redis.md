...
- string 自增（incre）、分布式锁（setnx）、seesion共享、
  ...
  - 主从同步数据丢失
    - 红锁：部署多套redis环境，每个环境中有一个redission node，多个redission node组成红锁
    - 加锁示只有半数以上枷锁成功才算成功
    - 加锁时间超过指定时间就会失败
...
#### 单线程的高性能
- 基于内存
- 执行命令单线程，持久化、异步删除、集群数据同步
- I/O多路复用
- Redis 的线程模型：基于Reactor模式开发了自己的网络事件处理器，即文件事件处理器
- ![](./pic/filehandler.png)
...
后对16384取模得到具体槽位
- 可以指定集群以及节点ip通过reshard分配节点
- 通过hash tag可以将不同的key映射同一个同一个hash slot里
...
- 节点间通信协议：gossip，
  - 维护集群的元数据（集群节点、主从角色、节点数量，、各节点共享的数据）有两种方式
    ...
    - gossip协议包含多种信息，ping、pong、meet、fail等。
      ...
      - 到消息的节点不回传散播：A -> B，那么B进行散播的时候，不再发给 A。
...
- 对象类型 基于以上数据结构创建的对象系统
  ...
  - ![](./pic/zset_ele1.png)
    ...
    -字典和跳跃表 
      ...
      - 层：保存着指向其他元素的指针。高层的指针越过的元素数量大于等于低层的指针，为了提高查找的效率， 
      程序总是从高层先开始访问，然后随着元素值范围的缩小，慢慢降低层次。
...
- 热点缓存key的重建优化
  - 分布式锁保证只有一个线程能重构
- 双写不一致 
  - [不一致](https://mp.weixin.qq.com/s/ooxSSQA-o2Vuk_BCvuP6Lg).
  - 加过期时间
  - 串行执行
  - 延时双删：删除后睡眠一段时间再删除一次
  - 分布式锁串行，redission读写锁
...
- 删除策略
  ...
  - 定期删除 
    - redis每过100ms，轮询所有的数据库
    - 从设置了过期时间的key中随机取出20个缓存key
    - 清除其中的过期key
    - 若过期key占比超过1/4，则重复步骤2
